...
constexpr unsigned int max_num_streamers = 10;

struct streamer_t {
   virtual ~streamer_t() {}
   virtual void start_block(uint32_t block_num, uint32_t streamer_id) {};
   virtual void stream_data(const char* data, uint64_t data_size, uint32_t streamer_id) = 0;
   virtual void stop_block(uint32_t block_num, uint32_t streamer_id) {}
};
