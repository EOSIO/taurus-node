...
public:
   ...
   struct whitelist_blacklist {
      std::optional< boost::container::flat_set<chain::account_name> > actor_whitelist;
      std::optional< boost::container::flat_set<chain::account_name> > actor_blacklist;
      std::optional< boost::container::flat_set<chain::account_name> > contract_whitelist;
      std::optional< boost::container::flat_set<chain::account_name> > contract_blacklist;
      std::optional< boost::container::flat_set< std::pair<chain::account_name, chain::action_name> > > action_blacklist;
      std::optional< boost::container::flat_set<chain::public_key_type> > key_blacklist;
   };

   struct greylist_params {
      std::vector<chain::account_name> accounts;
   };
   ...
   struct get_account_ram_corrections_params {
      std::optional<chain::account_name>  lower_bound;
      std::optional<chain::account_name>  upper_bound;
      uint32_t                     limit = 10;
      bool                         reverse = false;
   };
   ...
   struct get_account_ram_corrections_result {
      ...
      std::optional<chain::account_name>  more;
   };
   ...
   void log_failed_transaction(const chain::transaction_id_type& trx_id, const chain::packed_transaction_ptr& packed_trx_ptr, const char* reason) const;
   ...
   void create_snapshot(chain::plugin_interface::next_function<snapshot_information> next);
...
FC_REFLECT(eosio::producer_plugin::runtime_options, (max_transaction_time)(max_irreversible_block_age)(produce_time_offset_us)(last_block_time_offset_us)(subjective_cpu_leeway_us)(greylist_limit));
