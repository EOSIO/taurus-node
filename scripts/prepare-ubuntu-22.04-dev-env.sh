...
apt update -y
apt install -y curl gnupg bzip2 python3-pip cmake libgmp-dev pkg-config colordiff libusb-1.0-0-dev libcurl4-openssl-dev autoconf libtool-bin
...
export PATH=/usr/lib/llvm-13/bin/:/usr/lib/llvm-11/bin:$PATH
...
curl -fsSLOk "https://boostorg.jfrog.io/artifactory/main/release/${BOOST_VERSION_DOT}/source/boost_${BOOST_VERSION}.tar.bz2" && \
...
curl -fsSLOk https://github.com/protocolbuffers/protobuf/releases/download/v${PROTOBUF_VERSION_SHORT}/protobuf-cpp-${PROTOBUF_VERSION}.tar.gz && \
...
echo ""
echo "  export PATH=/usr/lib/llvm-13/bin/:/usr/lib/llvm-11/bin:\$PATH"
