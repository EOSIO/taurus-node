...
# we do not start producer and SHiP.
#
###############################################################
...
def stopRodeos():
    ...
    subprocess.call(("pkill -9 rodeos").split())
    if rodeosStdout:
        rodeosStdout.close()
    if rodeosStderr:
        rodeosStderr.close()
...
def startRodeos(args = []):
    ...
    subprocess.Popen(['./programs/rodeos/rodeos', '--rdb-database', os.path.join(rodeosDir,'rocksdb'), '--data-dir', rodeosDir] + args,
...
run_test(title = 'mixed legacy and multi filter contracts',
        ...
        expectedLogs = ['legacy and mulitiple filter contracts cannot be mixed']
        )
