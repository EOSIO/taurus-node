...
# 1: Kill -9 leader BP and cluster should choose new leader and resume block production
# 2: kill -9 one non-leader BP and cluster pauses production
...
try:
    ...
    # kill -9 leader BP
    if not Leader[0][1].kill(signal.SIGKILL):
    ...
    # kill -9 non-leader BP
    if not nonLeaders[0][1].kill(signal.SIGKILL):
        Utils.errorExit("Failed to shutdown node")
