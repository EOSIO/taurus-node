...
The EOSIO-Taurus blockchain persists the states as snapshots to replace the shared memory file state persist mechanism. The shared memory file solution has 2 main issues
...
- creates a snapshot during shutdown.
  - also, regularly, spawns background process with a copy of the process state making use the copy-on-write efficient memory cloning from `fork()`, to create a snapshot.
...
Temporary files named `.state_snapshot.bin` and `..state_snapshot.bin` may be also found there during shutdown or background time snapshot creation. They will be atomically renamed to `state_snapshot.bin` upon successful snapshot creation.
